...
- **Decline-memory.** At session start, read `${AMPLIFIER_WAYFINDER_DIR:-~/.amplifier/wayfinder}/declines.md` (create the dir/file if absent). Never re-offer any offer-id listed there. On a "no," append that offer-id to the file so it stays declined across sessions.
